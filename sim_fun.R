...
sim_fun2 = function(nsim, dist_c, dist_params, samples, f_2 = NULL, c_type, w_type = "unweighted"){
  ...
  for(z in 1:nrow(settings)){
    sets = settings[z,]
    sizes = nm_gen2(nn = sets$nn, n_i = sets$n_i, m_ij = sets$m_ij, each_s = sets$each_s, both_s = sets$both_s, identical_s = sets$identical_s, identical_c = sets$identical_c)
  ...
  names(results) = c("no_cor", "mild_cor", "sev_cor")
  save(results, file = paste0(w_type, "_",dist_c, "_mildU_results.RData"))
